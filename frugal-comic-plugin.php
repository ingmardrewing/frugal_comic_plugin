...
function fcp_write_navigation ( $min_post_id , $dom ){
  ...
  if( !( get_the_ID() < $min_post_id ) && ( $latest_cpt[0]['ID'] != get_the_ID() )) {
    $html .= "<a href='". get_permalink( $my_latest_post_id) ."'>newest &gt;&gt;</a>";
  }
  $html .= '</li>';
  $html .= '<li style="list-style-type:none;float:right;margin-right:10px;">';
  if( !( get_the_ID() < $min_post_id ) && ( $latest_cpt[0]['ID'] != get_the_ID() )) { 
    $next_issue_url = get_permalink(get_adjacent_post(false,'',false)) ;
    $html .= "<a rel='next' href='". $next_issue_url ."'>next &gt;</a>";
  }
  $html .= '</li>';
  $html .= '<li style="list-style-type:none;float:right;margin-right:10px;">';
  if( !( get_the_ID() < $min_post_id + 1 ) ) { 
    $prev_issue_url = get_permalink(get_adjacent_post(false,'',true))  ;
    $html .= "<a rel='prev' href='". $prev_issue_url ."'>&lt; previous</a>";
  }
  $html .= '</li>';
  $html .= '<li style="list-style-type:none;float:right;margin-right:10px;">';
  if( !( get_the_ID() < $min_post_id + 1 ) ) { 
    $start_issue_url = 'http://devabo.de/2013/08/01/a-step-in-the-dark/'  ;
    $html .= "<a rel='start' href='". $start_issue_url ."'>&lt;&lt; first</a>";
  }
  $html .= '</li>';
  $html .= '</ul></div>';
